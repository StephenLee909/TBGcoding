import numpy as np
import matplotlib.pyplot as plt
import os
import seaborn as sns
import time
from scipy.signal import argrelextrema

# 設定輸出資料夾
def setup_output_folder(folder_name, subfolders=None):
    if not os.path.exists(folder_name):
        os.makedirs(folder_name,exist_ok=True)
    
    if subfolders:
        for subfolder in subfolders:
            subfolder_path = os.path.join(folder_name, subfolder)
            if not os.path.exists(subfolder_path):
                os.makedirs(subfolder_path,exist_ok=True)
                
    return folder_name

# 設定參數
def constants():
    a = 2.464  # lattice constant (Å)
    v = np.sqrt(3) / 2 * a * 2.649  # Dirac velocity (J*Å) 5.65
    k_D = 4 * np.pi / (3 * a)  # Magnitude of BZ corner wave vector
    w = 110 * 10**-3  # hopping energy (meV)
    return v, k_D, w

# 定義Pauli矩陣
def pauli_matrices():
    s_0 = np.array([[1, 0], [0, 1]])
    s_1 = np.array([[0, 1], [1, 0]])
    s_2 = np.array([[0, -1j], [1j, 0]])
    return s_0, s_1, s_2

def theta_parameter(theta, v, k_D, w):
    k_theta = 2 * k_D * np.sin(theta / 2)
    q_ = k_theta * np.array([[0, 1], [-3**0.5 / 2, -0.5], [3**0.5 / 2, -0.5]])
    alpha = w / (v * k_theta)
    b_ = np.array([q_[0]-q_[2],q_[1]-q_[2]])
    return k_theta, q_, alpha, b_

# K point
def generate_kpts(k_paths, num_kpts):
    k_points = [((1 - i / num_kpts) * start + (i / num_kpts) * end) for start, end in k_paths for i in range(num_kpts)]
    return np.array(k_points)

# Q point
def generate_Qpts(vector,b0,b1,length):
    vectors_set = set()
    m = int(np.ceil(2*length/3))+2
    n = int(np.ceil((2*length+1)/3))+2
    for i in range(-m,n):
        for j in range(-m,n):
            if np.linalg.norm(vector+i*b0+j*b1) <= length:
                vectors_set.add(tuple(vector+i*b0+j*b1))
    return np.array(list(vectors_set))

# Convergence length calculation
def calculate_convergence_length(theta_range):
    est_Qnum = 5/np.degrees(np.min(theta_range))**2
    est_length = int(np.rint((est_Qnum*3**1.5/np.pi)**0.5))
    print(f'Length is {est_length}')
    return est_length

# Hamiltonian計算
def h(k, Q,s_1,s_2):
    return (k[0] - Q[0]) * s_1 + (k[1] - Q[1]) * s_2

def T(i, j, Q, q_nn, s_0, s_1, s_2):
    difference = Q[i] - Q[j]
    norm_diffs = [np.linalg.norm(difference - q_nn[k]) for k in range(6)]
    min_index = np.argmin(norm_diffs)

    if norm_diffs[min_index] < 10**-6:
        if min_index < 2:
            return s_0 + s_1
        elif min_index < 4:
            return s_0 - 0.5 * s_1 + (3**0.5 / 2) * s_2
        else:
            return s_0 - 0.5 * s_1 - (3**0.5 / 2) * s_2
    return np.zeros((2, 2))

def hamiltonian(k, Q, q_nn, v, w, s_0, s_1, s_2):
    num_Qpts = Q.shape[0]
    H = np.zeros((num_Qpts*2, num_Qpts*2), dtype=complex)
    # 填入上三角部分
    for i in range(num_Qpts):
        for j in range(i + 1, num_Qpts):
            H[2 * i: 2 * i + 2, 2 * j: 2 * j + 2] = w * T(i, j, Q, q_nn, s_0, s_1, s_2)
    
    # 填充下三角部分
    H += np.conjugate(H).T
    
    # 填對角線
    for i in range(num_Qpts):
        H[2 * i: 2 * i + 2, 2 * i: 2 * i + 2] = -v * h(k, Q[i], s_1, s_2)
    
    return H

# 計算能帶結構
def calculate_band_structure(k_points, Q, q_nn, v, w, s_0, s_1, s_2):
    energies = []
    eigenvectors_list = []
    
    for k in k_points:
        H = hamiltonian(k, Q, q_nn, v, w, s_0, s_1, s_2)
        eigenvalues, eigenvectors = np.linalg.eigh(H)
        energies.append(eigenvalues)
        eigenvectors_list.append(eigenvectors)
    
    return np.array(energies), np.array(eigenvectors_list)

# 繪製能帶結構
def plot_band_structure(energies, k_points, theta, fermi_energy, energy_range, output_folder):
    plt.figure(figsize=(6, 10))
    # 設定字型大小
    plt.rcParams['font.size'] = 14  # 設定全域的字型大小
    plt.rcParams['axes.titlesize'] = 35  # 設定標題的字型大小
    plt.rcParams['axes.labelsize'] = 25  # 設定坐標軸標籤的字型大小
    plt.rcParams['xtick.labelsize'] = 35  # 設定x軸刻度的字型大小
    plt.rcParams['ytick.labelsize'] = 15  # 設定y軸刻度的字型大小

    #邊緣
    plt.subplots_adjust(left=0.21, right=0.95, top=0.90, bottom=0.1)
    for i in range(energies.shape[1]):
        band = energies[:, i]
        if np.any((band > fermi_energy - energy_range) & (band < fermi_energy + energy_range)):
            plt.plot(band)
    
    # 添加標記點
    k_labels = ['K\'', 'K', 'Γ\'', 'Γ', 'K\'']
    k_indices = [0, len(k_points)//4, len(k_points)//2, 3*len(k_points)//4, len(k_points) - 1]
    for index in k_indices:
        plt.axvline(x=index, color='k', linestyle='--', linewidth=0.5)
    plt.axhline(y=0, color='k', linestyle='--', linewidth=0.5)
    plt.xticks(k_indices, k_labels)

    # Plt Axis
    plt.ylabel('Energy(ev)')
    plt.title(f'θ={theta*180/np.pi:.4f} degrees')
    plt.xlim(0,len(k_points)-1)
    plt.ylim(-energy_range,energy_range)
    
    # Save
    filename = f'{output_folder}/band_structure/band_structure_{theta*180/np.pi:.4f}_degrees.png'
    plt.savefig(filename)
    plt.show()
    plt.close()

# 繪製Hamiltonian的熱圖
def plot_hamiltonian_heatmap(H, output_folder,theta):
    Re_H = H.real
    Im_H = H.imag
    plt.figure(figsize=(18, 18))
    sns.heatmap(Re_H, annot=False, cmap='coolwarm')
    plt.title('Heatmap of the Real Part Hamiltonian')
    filename = f'{output_folder}/hamiltonian_heatmap/Real Part Hamiltonian_{theta*180/np.pi:.2f}_degrees.png'
    plt.savefig(filename)
    plt.close()
    
    plt.figure(figsize=(18, 18))
    sns.heatmap(Im_H, annot=False, cmap='coolwarm')
    plt.title('Heatmap of the Imaginary Part Hamiltonian')
    filename = f'{output_folder}/hamiltonian_heatmap/Imaginary Part Hamiltonian_{theta*180/np.pi:.2f}_degrees.png'
    plt.savefig(filename)
    plt.close()

# Plot the k or Q point
def plot_point(points, output_folder,Title):
    plt.figure(figsize=(6, 6))
    plt.scatter(points[:, 0], points[:, 1], color='blue', marker='o')
    # 為每個點添加標籤
    # for i, point in enumerate(points):
    #     plt.text(point[0], point[1], str(i), fontsize=9, ha='right')
    plt.xlabel('X coordinate')
    plt.ylabel('Y coordinate')
    plt.title(f'{Title}_# of total pts = {points.shape[0]}')
    plt.grid(True)
    plt.axis('equal')  # 保證 x 與 y 軸相同比例
    filename = f'{output_folder}/plot_points/{Title}.png'
    plt.savefig(filename)
    plt.close()

# 計算能帶斜率絕對值，再取平均
def calculate_band_slope(energies, k_points, band_indices):
    slopes = []
    dk = np.linalg.norm(k_points[1] - k_points[0]) # 每個k間距離都等長，故取第二個跟第一個的距離
    for i in band_indices:
        band = energies[:, i]
        slope = np.gradient(band, dk)
        slopes.append(np.abs(slope))
    return np.mean(slopes)

# Plot velocity v.s theta(degree) and alpha_square
def plot_velocity(theta_range,theta_slopes,alpha_range,output_folder,type):
    # 繪製不同角度的斜率圖
    plt.figure(figsize=(8, 6))
    theta_degrees = np.degrees(theta_range)
    match type:
        case 0:
            plt.plot(theta_degrees, theta_slopes)
            plt.xlabel('Theta (degrees)')
            plt.ylabel('Average Band Slope')
            plt.title('Average Band Slope vs. Theta')
            plt.grid(True)
            plt.savefig(f'{output_folder}/velocity/v_vs_theta_{theta_degrees[-1]:.2f}-{theta_degrees[0]:.2f}.png')
            plt.close()

        case 1:
            alpha_range = np.array(alpha_range)
            theta_slopes = np.log10(theta_slopes)
            minima_idx = argrelextrema(theta_slopes, np.less)[0]
            plt.plot(alpha_range, theta_slopes, marker='o')
            # 在相對極小值處標記
            plt.plot(alpha_range[minima_idx], theta_slopes[minima_idx], 'ro', label='Local Minima')
            # 標示相對極小值的座標
            for i in minima_idx:
                plt.text(alpha_range[i], theta_slopes[i], f'({alpha_range[i]:.4f}, {theta_slopes[i]:.2f})', fontsize=9, ha='right')
            plt.xlabel('Alpha')
            plt.ylabel('Average Band Slope')
            plt.title('Average Band Slope vs. Alpha')
            plt.legend()
            plt.grid(True)
            plt.savefig(f'{output_folder}/velocity/log(v)_vs_alpha_{alpha_range[0]:.2f}-{alpha_range[-1]:.2f}.png')
            # plt.show()
            plt.close()

        case 2:
            alpha_square = np.array(alpha_range) @ np.array(alpha_range)
            plt.plot(alpha_square, theta_slopes)
            plt.xlabel('Alpha Square')
            plt.ylabel('Average Band Slope')
            plt.title('Average Band Slope vs. Alpha Square')
            plt.grid(True)
            plt.savefig(f'{output_folder}/velocity/v_vs_alpha_square_{alpha_square[0]:.2f}-{alpha_square[-1]:.2f}.png')
            plt.close()

# Calculate Charge  
def chrage_indicies(num_Qpts, num_QT):
    TA = list(range(0, 2*num_QT, 2))
    TB = list(range(1, 2*num_QT, 2))
    BA = list(range(2*num_QT, 2*num_Qpts, 2))
    BB = list(range(2*num_QT+1, 2*num_Qpts, 2))
    return [TA, TB, BA, BB]

def chrage_func(indicies, k_point, eigenvector, Q, r):
    z = 0
    for i in indicies:
        z += eigenvector[i] * np.exp(1j*(np.dot(Q[int(np.floor(i/2)),:]-k_point,r)))
        # z += eigenvector[i]*np.conjugate(eigenvector[i]) test for vector normalization
    return np.abs(z)**2

def plot_charge(charge_indicies, band_indices, k_points, eigenvectors, Q, 
                output_folder, theta, plot_type, Super_Lacttice_length):

    # 使用元組作為字典的鍵
    plot_indicies = [(0,),(2,),(0,1),(2,3),(0,2),(0,1,2,3)]
    plot_indicies_name = ["TA","BA","TOP","BOT","A","TOT"]
    plot_indicies_dict = dict(zip(plot_indicies, plot_indicies_name))

    plot_k_points = [(0,),(1,),(0,1)]
    plot_k_points_name = ["K","K\'","K+K\'"]
    plot_k_points_dict = dict(zip(plot_k_points, plot_k_points_name))
    
    # 設置參數
    num_points = int(np.rint(Super_Lacttice_length))   # 網格點的數量
    bound = Super_Lacttice_length
    x = np.linspace(-bound, bound, num_points)      # x軸的範圍
    y = np.linspace(-bound, bound, num_points)      # y軸的範圍

    # 生成網格點
    X, Y = np.meshgrid(x, y)

    # 計算每個網格點的Charge值
    Z_tot = np.zeros(X.shape, dtype=float)
    for j in range(num_points):
        for k in range(num_points):
            r = np.array([X[j, k], Y[j, k]])
            a = np.sum([
                chrage_func(charge_indicies[i],k_points[m], eigenvectors[m, :, l], Q, r)
                for l in band_indices for i in plot_indicies[plot_type[0]] for m in plot_k_points[plot_type[1]]
            ])
            Z_tot[j, k] = np.log10(np.abs(a))

    # 繪製等高線圖
    plt.contourf(X, Y, Z_tot, levels=80, cmap='viridis')
    plt.colorbar(label='Charge Amplitude')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.title('Charge distribution take Log')

    # 使用 plot_type 來訪問字典值，並生成文件名
    outputname =f'{output_folder}/chrage_func/{theta*180/np.pi:.4f}_{plot_indicies_dict[tuple(plot_indicies[plot_type[0]])]}_{plot_k_points_dict[tuple(plot_k_points[plot_type[1]])]}.png'
    plt.savefig(outputname, dpi = 800)

    plt.close()
    
def main():

    # 常數
    v, k_D, w = constants()
    s_0, s_1, s_2 = pauli_matrices()

    # Thetarange input
    m_range = np.arange(30,31) 
    theta_range = np.arccos((3*m_range**2+3*m_range+0.5)/(3*m_range**2+3*m_range+1))
    Super_Lacttice_length = 2.464/(2*np.sin(theta_range[-1]/2))
    theta_slopes = []    # 保存不同角度的斜率25
    alpha_range = []
    
    length = calculate_convergence_length(theta_range) # in the unit of 1/Anstrong
    output_folder = f'radius={length}'
    subfolders = ['band_structure', 'hamiltonian_heatmap', 'plot_points','velocity','chrage_func']
    setup_output_folder(output_folder, subfolders)
    print("The Output folder is "f'{output_folder}')

    # Define Qpts .
    q_ = np.array([[0,1],[-3**0.5 / 2, -0.5], [3**0.5 / 2, -0.5]]) #q_ is unit vector
    b_ = np.array([q_[0]-q_[1],q_[0]-q_[2]])
    Q_T = generate_Qpts(-q_[1], b_[0], b_[1], length)
    Q_B = generate_Qpts(np.zeros(2), b_[0], b_[1], length)
    Q = np.concatenate((Q_T, Q_B), axis=0)
    num_Qpts = Q.shape[0]
    band_indices = [num_Qpts-1,num_Qpts] #中間2條離0最近

    # Charge input
    plot_charge_type = [0,2]    #0:TA       #0:K only
                                #1:BA       #1:K' only
                                #2:TOP      #2:Both
                                #3:BOTTOM
                                #4:A
                                #5:TOTAL
    charge_indicies = chrage_indicies(num_Qpts, Q_T.shape[0])

    # Band Diagram input 
    fermi_energy = 0  # 假設費米能級為0
    energy_range = 0.005  # 設定顯示的能帶範圍

    # PLot velocity input
    plot_velocity_type = 1  #0:v vs theta 
                            #1:log(v/vf) vs alpha
                            #2:v vs alpha_square

    # Number of Kpoint
    num_kpts = 40    #1:Plot Charge 
                    #4:Plot Velocity
                    #20:Plot Band
    
    for theta in theta_range:
        k_theta, q_, alpha, b_ = theta_parameter(theta, v, k_D, w)
        Q_t = Q * k_theta 
        q_nn = np.array([q_[0],-q_[0],q_[1],-q_[1],q_[2],-q_[2]])

        k_paths = [                 # Path that used to calculate Band
            (-q_[0], np.zeros(2)),  # A->B
            (np.zeros(2), q_[0]),   # B->C
            (q_[0], q_[2]),         # C->D
            (q_[2], -q_[0])         # D->A
        ]

        # k_paths = [                 # Path that used to calculate Velocity (Near B) and chrage_func
        #     (-0.1*q_[0], 0.1*q_[0])
        # ]

        k_points = generate_kpts(k_paths, num_kpts)

        # k_points = np.array([[0,0],-q_[0]]) # K point used to calculate chrage_func

        # # Plot K-points
        plot_point(k_points,output_folder,"K-points")

        # # Plot Q-points
        # plot_point(Q_t,output_folder,"Q-points")

        # # Hamiltonian_heatmap
        # plot_hamiltonian_heatmap(hamiltonian(np.array([0,0]), Q_t,q_nn, v, w, s_0, s_1, s_2,num_Qpts), output_folder,theta)

        # Construct H
        energies , eigenvectors = calculate_band_structure(k_points, Q_t, q_nn, v, w, s_0, s_1, s_2)

        # 繪製能帶結構
        plot_band_structure(energies, k_points, theta, fermi_energy, energy_range, output_folder)

        # # Calculate The chrage_func Density
        # for plot_charge_type in range(6):
        #     for k_points_type in range(3):
        #         plot_charge(charge_indicies, band_indices, k_points, eigenvectors, Q_t, 
        #                 output_folder, theta, [plot_charge_type,k_points_type], Super_Lacttice_length)

    #     # 計算能帶斜率平均
    #     slopes = calculate_band_slope(energies, k_points, band_indices)
    #     theta_slopes.append(slopes)
    #     alpha_range.append(alpha)

    # theta_slopes = np.array(theta_slopes/v)
    # plot_velocity(theta_range,theta_slopes,alpha_range,output_folder,plot_velocity_type) 

    # Convergence Test
    # for m in range(1,300,10):
    #     theta_range = np.arccos((3*m**2+3*m+0.5)/(3*m**2+3*m+1))
    #     print(f'{m}'+'\t'+f'{calculate_convergence_length(theta_range)}'+'\t'+f'{2.464/(2*np.sin(theta_range/2)):.2f}')


if __name__ == "__main__":
    start_time = time.time()
    main()
    end_time = time.time()
    print(f"time: {end_time - start_time:.2f} s")

        
# 建立 band class
# 計算函數: 有 calculate_band_structure, calculate_band_slope
# 畫圖函數: plot_band_structure, plot_velocity
# 變數: length, 常數, theta部分, Q部分