...
class TBG:
    ...
    def __init__(self, m_range: np.ndarray, k_points: np.ndarray ) -> None:
        self.__theta_range = np.arccos((3*m_range**2+3*m_range+0.5)/(3*m_range**2+3*m_range+1))
        self.__k_points = k_points
        
class K_PATH:

    def __init__(self,path: str, num_kpts: int) -> None:
        self.path = generate_kpts()
...
def plot_band_structure(energies, k_points, theta, fermi_energy, energy_range, output_folder):
    ...
    for i in range(energies.shape[1]):
        ...
        if np.any((band > fermi_energy - energy_range) & (band < fermi_energy + energy_range)):
            plt.plot(band, label=f'Band {i + 1}')
    ...
    plt.title(f'Band Structure for θ={theta*180/np.pi:.4f} degrees')
...
def main():
    ...
    for theta in theta_range:
        ...
        # Construct H
        energies , eigenvectors = calculate_band_structure(k_points, Q_t, q_nn, v, w, s_0, s_1, s_2,num_Qpts)
